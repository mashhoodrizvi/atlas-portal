  ...
  if(!filtered.length) {
    ...
    body.innerHTML=filtered.map((r,i)=>{
      const cells=columns.map(c=>{
        if(c==="Amount") return `<td class="amount">${money(r[c])}</td>`;
        if(c==="Status") return `<td><span class="status ${statusClass(r[c])}">${esc(r[c]||"Not set")}</span></td>`;
        if(durationColumns.has(c)) return `<td>${esc(duration(r[c]))}</td>`;
        return `<td>${esc(r[c]||"—")}</td>`;
      }).join("");

      const actions=adminPassword
        ? `<td class="actions-cell">${String(r.Status).toLowerCase()==="pending"
...
function updateStats() {
  ...
  $("#totalAmount").textContent=money(filtered.reduce((s,r)=>s+(Number(String(r.Amount??"").replace(/[$,]/g,""))||0),0));
}
