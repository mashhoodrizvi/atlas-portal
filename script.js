...
let nextCustomId = workItems.length + 1;
...
function renderItems() {
  itemsList.innerHTML = state.map((item, index) => {
    ...
    const itemLabel = item.title.trim() || `added item ${itemNumber}`;
    const description = item.custom
      ? `
        <div class="item-description custom-item-description">
          <div class="custom-item-title-row">
            <span class="custom-item-number" aria-hidden="true">${String(itemNumber).padStart(2, "0")} ·</span>
            <label class="custom-field">
              <span class="sr-only">Item ${itemNumber} name</span>
              <input
                class="custom-item-input custom-title-input"
                type="text"
                data-action="title"
                data-index="${index}"
            ...
            <button
              ...
              data-action="remove"
              data-index="${index}"
              aria-label="Remove added item ${itemNumber}"
            >Remove</button>
          </div>
          <label class="custom-field">
            <span class="sr-only">Item ${itemNumber} description</span>
            <textarea
              class="custom-item-input custom-detail-input"
              data-action="detail"
              data-index="${index}"
              rows="2"
      ...
      : `
        <div class="item-description">
          <strong>${String(itemNumber).padStart(2, "0")} · ${escapeHtml(item.title)}</strong>
          <span>${escapeHtml(item.detail)}</span>
        </div>`;

    return `
    <article class="item-row ${item.selected ? "is-selected" : ""} ${item.custom ? "is-custom" : ""}" data-item-id="${item.id}">
      <label class="item-check-wrap" aria-label="Include ${escapeHtml(itemLabel)}">
...
itemsList.addEventListener("input", event => {
  ...
  if (action === "title" || action === "detail") {
    if (state[index]) state[index][action] = target.value;
    return;
  }
...
get("addItemBtn").addEventListener("click", () => {
  const newItemIndex = state.length;
  state.push({
    id: nextCustomId,
    title: "",
    detail: "",
  ...
  requestAnimationFrame(() => {
    itemsList.querySelector(`.custom-title-input[data-index="${newItemIndex}"]`)?.focus();
  });
});
...
function validateQuote() {
  ...
  const unnamedCustomItems = selected.filter(item => item.custom && !item.title.trim());
  unnamedCustomItems.forEach(item => {
    const index = state.indexOf(item);
    itemsList.querySelector(`.custom-title-input[data-index="${index}"]`)?.classList.add("invalid");
  });
  ...
  if (unnamedCustomItems.length) {
    showMessage("Enter an item name for every selected added item.");
    const firstIndex = state.indexOf(unnamedCustomItems[0]);
    itemsList.querySelector(`.custom-title-input[data-index="${firstIndex}"]`)?.focus();
    return false;
  }
...
async function downloadPdf() {
  ...
  const tableBody = [
    ...
    ...totals.selected.map((item, index) => [
      String(index + 1),
      [item.title, item.detail, `Quantity: ${item.quantity}    Unit price: ${formatMoney(item.price)}`].filter(Boolean).join("\n"),
      formatMoney(lineTotal(item))
    ]),
...
get("resetBtn").addEventListener("click", () => {
  ...
  state.splice(0, state.length, ...workItems.map(item => ({
    ...item,
    custom: false,
    selected: false,
    quantity: 1,
  ...
  nextCustomId = workItems.length + 1;
  hideMessage();
  renderItems();
  calculateTotals();
